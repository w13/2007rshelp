...
<div style="margin:1pt; font-size:large; font-weight:bold;">&raquo; <a href="<?=$_SERVER['SCRIPT_NAME']?>">Runescape Item Database</a></div>
...
<p><b>&raquo;</b>&nbsp;<a href="/items.php?search_area=type&amp;search_term=1">Armour and weapons</a>, <a href="/items.php?search_area=type&amp;search_term=2">Quest</a> and <a href="/items.php?search_area=type&amp;search_term=3">General</a> categories<br /><b>&raquo;</b> <a href="javascript:addEngine('items','gif','Items','0')" title="Click Here to Install FireFox Item DB Search Addon">FireFox Search Engine</a> Addon<br /><b>&raquo;</b> Tips on what you should do with your quest items<br /><b>&raquo;</b> Market Prices, Low and High Alchemy prices<br /><b>&raquo;</b> Extensive notes on each item<br /><b>&raquo;</b> A javascript tool to calculate amounts of items you're trading<br /><b>&raquo;</b> ... and loads more!</p>
<br />
...
<td style="vertical-align:middle;text-align:right;"><a href="javascript:addEngine(\'items\',\'gif\',\'Items\',\'0\')" title="Click Here to Install FireFox Item DB Search Addon">FF Search Addon</a> &nbsp; <a href="/correction.php?area=items&amp;id=4296">Submit Missing Item</a></td></tr></table><br />
  ';
...
$info['trade'] = $info['trade'] == 1 ? 'Yes' : 'No';
    echo '<tr>';
    echo '<td class="tablebottom"><a href="' . $_SERVER['SCRIPT_NAME'] . '?id=' . $info['id'] . '&amp;runescape_' . $seotitle . '.htm"><img src="/img/idbimg/' . $info['image'] . '" alt="Zybez Runescape Help\'s ' . $info['name'] .' image" width="50" height="50" /></a></td>';
...
&raquo; <a href="<?=$_SERVER['SCRIPT_NAME']?>">Item Database</a> &raquo; <u><?=$info['name']?></u></div>
<hr class="main" noshade="noshade" />
...
$questlist = substr($questlist, 0, -2);
  ...
  elseif(stripos($info['name'],'lamp')>0) {
    ...
    $mquery = $db->query("SELECT name, id FROM monsters WHERE id NOT IN (3488) AND (drops LIKE '%" . $search . "%' OR i_drops LIKE '%" . $search . "%') GROUP BY name ORDER BY combat ASC");
  }
  
  $s=0;
  while($minfo = $db->fetch_array($mquery)) {
        $s++;
        if($s<16) $droplist .= '<a href="/monsters.php?id='.$minfo['id'].'" title="Runescape Monster: '.$minfo['name'].'">'.$minfo['name'].'</a>, ';
  }
  
  /*Display Monsters that drop this item */
  $more = '';
  if(@mysql_num_rows($mquery) > 15 && !empty($droplist)) {
...
$picture = @mysql_result($db->query("SELECT p.jagex_pid FROM price_items p JOIN items i ON p.id=i.pid WHERE i.id = " . $id),0);
...
if($info['trade'] == "Yes") {
    echo '<tr><td width="35%">Market Price:</td><td width="70%">'.$pricelist.'</td></tr>';
    echo '<tr><td>High Alchemy:</td><td>' . number_format($info['highalch']) . 'gp</td></tr>';
    ...
    echo '<tr><td>High Alchemy:</td><td>' . $info['highalch'] . 'gp</td></tr>';
    ...
    echo '<tr><td class="boxbottom">Stab: '.$def[0].'<br />Slash: '.$def[1].'<br />Crush: '.$def[2].'<br />Magic: '.$def[3].'<br />Range: '.$def[4].'<br />Summoning: '.$def[5].'</td></tr><tr><td style="height:4px"></td></tr>';
...
if(!empty($droplist)) {
    ...
    echo '<table cellspacing="0" cellpadding="4" style="width:76%;margin:0 12%;border: 1px solid #000">';
if($info['trade'] == "Yes") {
    echo '<tr><td width="25%">Market Price:</td><td>'.$pricelist.'</td></tr>';
}
else {
    echo '<tr><td width="25%">Market Price:</td><td>Not Applicable</td></tr>';
}    echo '<tr><td>High Alchemy:</td><td>' . number_format($info['highalch']) . 'gp</td></tr>';
    echo '<tr><td>Low Alchemy:</td><td>' . number_format($info['lowalch']) . 'gp</td></tr>';
    echo '</table><br />';
    echo '<table cellspacing="0" cellpadding="4" style="width:76%;margin:0 12%;border: 1px solid #000">';
