...
require( ROOT . '/' . 'calcs_functions.inc.php' );
...
<hr noshade="noshade" />
<p>Want to record your Runescape stat history? You've come to the right place! Everytime you use the Zybez <a href="calcs.php">Runescape Calculators</a> or the calculators in <a href="tiko.php">Tiko</a>, we record your latest stat information.
<p>We currently have over 2,000,000 stat histories for over 380,000 different players, recorded in our database.</p>
<?php
$modes = array('r','l','x');	## RANK, LEVEL, XP

$showduplicates = $_GET['showduplicates'];

if(!isset($_GET['from']) || $_GET['from']==''){ $from = 'last month'; }else{ $from = $_GET['from']; }
if(!isset($_GET['to']) || $_GET['to']==''){ $to = 'now'; }else{ $to = $_GET['to']; }

	echo '<form name="form" action="' . $_SERVER['SCRIPT_NAME'] . '">'
...
if(isset($_GET['Z-Scores'])) {
  ...
  else $list = 'WHERE `User`= "'.$username.'" '.$between.' ORDER BY `time` DESC'; //if($showduplicates=='yes')
...
$page_links = ($page > 1 AND $page < $page_count) ? '|' : '';
  ...
  /* Data */
		while($info = $db->fetch_array($query)) {
			...
			else echo '<td class="tabletop">' . date("d.m.y",$info['Time']) . '</td>';
			for($i=0;$i<count($skillnames);$i++) {
						if($info[($skillnames[$i].'r')] == '-1' || $info[($skillnames[$i].'l')] == '-1') {
							$info[($skillnames[$i].'l')] = '-';
							$title = 'Unknown';
							$expgains[$skillnames[$i]][] = 0;
						}
						else {
							$title = 'XP: '.number_format($info[($skillnames[$i].'x')]).' | Rank: '.number_format($info[($skillnames[$i].'r')]);
							$expgains[$skillnames[$i]][] = $info[($skillnames[$i].'x')];
						}
  ...
  if($row_count['count'] > 0 && !isset($_GET['Z-Scores'])){ /* Exp Gains Row Display */
		
		echo '<td class="tabletop">Exp. Gains</td>';
		for($i=0;$i<count($skillnames);$i++) {
			echo '<td onclick="javascript:alert(\'' . number_format($expgains[$skillnames[$i]][0]-($expgains[$skillnames[$i]][count($expgains[$skillnames[$i]])-1])) . ' exp total gained.\')">click</td>';
		}
  }
  if($row_count['count'] > $rows_per_page) echo '<tr><td colspan="27">'.$page_links.'</td></tr>';
	echo '</table></div>';
	echo '<p style="text-align:center;">
	<a href="' . $_SERVER['SCRIPT_NAME'] . '?grabstats='.$username.'" title="You may only update once an hour">Update '.$username.'\'s Stats from Runescape.com Now</a></p>';
