...
&raquo; <a href="<?php echo $_SERVER['SCRIPT_NAME']; ?>">OSRS RuneScape Item Database</a> &raquo; <u><?php echo $info['name']; ?></u></div>
<hr class="main" noshade="noshade" />
...
$picture_row = $db->fetch_row("SELECT p.jagex_pid FROM price_items p JOIN items i ON p.id=i.pid WHERE i.id = " . $id);
if(!empty($picture_row)) $big_pic = '<img src="/img/idbimgb/' . $picture_row['jagex_pid'] . '.gif" alt="Picture of ' . $info['name'] . '" />';
else $big_pic = '&nbsp;';
