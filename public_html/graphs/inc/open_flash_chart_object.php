...
function _ofc( $width, $height, $url, $use_swfobject, $base )
{
	...
	$out[] = 'var so = new SWFObject("'. $base .'osrs_help-runescape-help-graphs.swf", "'. $obj_id .'", "'. $width . '", "' . $height . '", "9", "#FFFFFF");';
    ...
    $out[] = '<param name="movie" value="'. $base .'osrs_help-runescape-help-graphs.swf?width='. $width .'&height='. $height . '&data='. $url .'" />';
    $out[] = '<param name="quality" value="high" />';
    $out[] = '<param name="bgcolor" value="#FFFFFF" />';
    $out[] = '<embed src="'. $base .'osrs_help-runescape-help-graphs.swf?data=' . $url .'" quality="high" bgcolor="#FFFFFF" width="'. $width .'" height="'. $height .'" name="'. $obj_id .'" align="middle" allowScriptAccess="sameDomain" ';
