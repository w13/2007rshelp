<?php
$cleanArr = array(  array('id', $_GET['id'], 'int', 's' => '1,50')
				  );
/*** QUEST PAGE ***/
...
<?php
/*
if (apc_exists('listofskills')) {
	echo apc_fetch('listofskills');
}else{
*/
  ...
  while($info = $db->fetch_array($query))
   {
    ...
    $listofskills .= '<td class="tablebottom"><a href="?id=' . $info['id'] . '">' . $info['name'] . '</a></td>' . NL;
    $listofskills .= '<td class="tablebottom">' . $info['type'] . '</td>' . NL;
    $listofskills .= '<td class="tablebottom">' . $info['author'] . '</td>' . NL;
	$listofskills .= '<td class="tablebottom">'.date('M j, Y', $info['time']).'</td>'.NL;
    $listofskills .= '</tr>';
   }
   
   echo $listofskills;
  //  apc_store('listofskills', $listofskills);
// }
...
&raquo; <a href="skills.php">Runescape Skill Guides</a> &raquo; <u><?php echo $info['name']; ?></u></div>
<hr class="main" noshade="noshade" />
