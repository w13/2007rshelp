...
$my_upload->upload_dir = (dirname(__FILE__) . '/' . 'uploads/'); // "files" is the folder for the uploaded files (you have to create this folder)
...
if(isset($_POST['Submit'])) {
	...
	if ($my_upload->upload($new_name)) { // new name is an additional filename information, use this to rename the uploaded file
        ...
        $ses->record_act('Upload Images', 'Upload', $myname, $ip);
}
/// UPLOAD CODE -->
...
<?
$info = $db->fetch_row("SELECT * FROM `admin_pads` WHERE `file` = 'upload'");
$last = format_time( $info['time'] + 21600 );
if( isset( $_POST['text'] ) ) {
    $pad = addslashes( $_POST['text'] );
    $query = $db->query("UPDATE `admin_pads` SET `text` = '".$pad."', `time` = '".time()."' WHERE `file` = 'upload'");
    $ses->record_act('Upload Images', 'Edit', 'Notepad', $ip);
...
$path  = (dirname(__FILE__) . '/' . 'uploads/');
	...
	if($file  ==  "."  ||  $file  ==  ".."  ||  $file  ==  "index.php" || $file == "editor-uploads" ) 
	continue; 

		echo  "$file<br /><img src=\"uploads/$file\" alt=\"$file\" border=\"0\" /><hr /><br />\n";
	}
